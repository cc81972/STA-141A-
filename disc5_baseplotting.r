# Discussion 4
# This material was originally developed by Andrew Blandino
# Plotting with base R

# R can do plotting without any packages (excluding the `graphics` package which comes with R).
# The R manual (see https://cran.r-project.org/doc/manuals/r-release/R-intro.pdf, see Ch 12) specifies 
# three types of plotting features available:
# 1. High-level plots: creates a new plot (overriding an existing plot).
# 2. Low-level plots: adds plotting content to an *existing* plot.
# 3. Interactive: adds/extracts information to/from an *existing* plot usually by way of the mouse.
# We won't spend any time on this last one, since they are less important, but the functionality is there.

# Let's see some examples.

### High-level plotting functions

# first start with plot()

# plot() generates different plots based on the input given

## plot(x) for x numeric vector
x <- rnorm(50)
class(x)
plot(x)
# What is on the x-axis? How do we verify our guess?


## Scatter plots (x,y) numeric
...
### Exercise ###
# get a scatter plot like the last one but on one plot
# but with the automatic and manual observations *distinct*
# (Hint: try the points() function )
#   fake code:
#   plot(auto)
#   points(manual, col='red')
## Also good practice to add a legend() if it's helpful!
...
## Exercise ##
set.seed(77)
# run a simulation to see how the least squares line
# evolves through repeated generation of the response, y
# through the regression model y = 3 + 2*x + error, with x Fixed.
